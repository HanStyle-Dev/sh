        ...
        if systemctl is-active systemd-resolved > /dev/null 2>&1; then
            ...
            if [ "$OS" = "ubuntu" ] || [ "$OS" = "debian" ]; then
                ...
                if [ -f /etc/resolv.conf ]; then
                    cp /etc/resolv.conf /etc/resolv.conf.systemd.bak
                fi
                ...
                if ! ss -ulpn | grep -q ":53 "; then
                    ...
                    if ! timeout 5 nslookup google.com > /dev/null 2>&1; then
                        warning "修改systemd-resolved配置后，DNS解析不再工作"
                        warning "正在创建新的resolv.conf文件..."
...
main() {
    ...
    # 最后检查一次DNS解析
    printf "\n"
    printf "正在进行最终DNS解析检查...\n"
    if ! timeout 5 nslookup google.com > /dev/null 2>&1; then
        warning "系统DNS解析仍然存在问题"
        warning "正在尝试最后的修复..."
